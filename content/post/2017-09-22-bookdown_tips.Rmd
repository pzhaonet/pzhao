...
<!--
## 如何在文章里显示 `r knitr::inline_expr('pi')` 这样的 R 代码

这个问题在编写 R markdown 教程时经常遇见。如果把世界上的提问按诡异程度排序的话，这个问题肯定能进前10，因为我根本不知道这个问题如何提问，如何描述。中英文都不知道。最后我只能这么提问：

> 请问怎么输入'撇 r pi 撇'并且别让它编译出来 3.1415？

对此，knitr 有[说明](https://yihui.name/knitr/faq/)：用 `knitr::inline_expr()`。

我现在用的方法是：`r '\x60r \\x60r pi\\x60\x60'`

这个 `\x60` 就是那个小撇。这个相当于把要显示的代码区弄成个字符串，然后插进去。
