...
公式的问题迟迟未解决，是因为我看到人们在讨论往html文件里加 `Load mathjax` 代码这种解决方案，一看就头大，不想做。今天突然灵光一现，在 `config.toml` 里设置 `math = true`，然后运行 `blogdown::serve_site()`，Duang！帖子里的公式就都可以正常显示了！这里秀一个：
...
需要注意的是，如果运行 `serve_site()` 来预览网页，那么 RStudio 的内置浏览器并不能正常显示公式，但是点击浏览器左上角的 `Show in new window` 按钮，用外部浏览器打开，就可以正常显示了。

2. **参考文献**

两个多月前，就[有个哥们儿问](https://github.com/rstudio/blogdown/issues/66)“有没有可能在blogdown创建的网站里添加参考文献?”作者回答得干脆：“有。”两人对话就终止了，让我目瞪口呆。由于在[ blogdown 官方手册](http://bookdown.org/yihui/blogdown)里并未看到添加参考文献的方法，我一直以为这个功能尚在开发中，直到今天也没见动静，有些起疑，就随便试了一下，步骤跟在 bookdown 里一样：

- 把参考文献库 .bib 文件考到 `content\post\`.
- 在帖子 `.Rmd` 文档的 yml 头里声明 .bib 文件的名称和路径。这里我设成 `bibliography: [book.bib, packages.bib]`.
- 现在可以在帖子里引用 .bib 里的文献条目了，例如 `[@R-blogdown]`.
- 运行建站函数 `blogdown::build_site()`.

上面的引用，会在帖子里显示 [@R-blogdown] ，而帖子末尾显示参考文献列表。
